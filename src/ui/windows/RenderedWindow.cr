...
class RenderedWindow < Hedron::Application
  ...
  def draw
    ...
    pathtextbox = main.["pathtextbox"].as(Hedron::Entry)
    ...
    main["runbutton"].as(Hedron::Button).on_click do |button|
      ...
      outputtextbox.text += "\nSaving to #{pathtextbox.text}"

        p = Process.new(Scripts::SNATCH_SCRIPT.call(commandtextbox.text.to_s, pathtextbox.text.to_s), shell: true)
        p.wait

        outputtextbox.text += ""
        outputtextbox.text += "\nDone!"
        @window.not_nil!.message(title: "Snatching complete!", description: "The website #{commandtextbox.text} and all of it's assets have been saved to #{pathtextbox.text}.")
